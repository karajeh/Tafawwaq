...
export function SidebarLayout({
  ...
  return (
    <div className="relative isolate flex min-h-svh w-full bg-white max-lg:flex-col lg:bg-white dark:bg-zinc-900 dark:lg:bg-zinc-950">
      {/* Sidebar on desktop */}
      <div className="fixed inset-y-0 left-0 w-64 max-lg:hidden lg:border-r lg:border-r-zinc-950/5">
        {sidebar}
      </div>
      ...
      <main className="flex flex-1 flex-col lg:min-w-0 lg:pl-64">
        ...
        <div
          className={`min-w-0 lg:flex hidden ${
            teacherRoute || studentRoute ? "hidden" : "border-b  border-slate "
          }`}
        >
          {navbar}
        </div>
        <div className="grow  lg:bg-white lg:p-5 lg:shadow-sm dark:lg:bg-zinc-900 dark:lg:ring-white/10 pt-0">
          {children}
        </div>
      </main>
    </div>
